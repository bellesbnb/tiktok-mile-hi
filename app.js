...
function openCreator(id) {
  ...
  $('#modalContent').innerHTML = `
    ...
    <div class="mt-8 space-y-3">
      ...
      ${(Array.isArray(c.tiktokAlts) ? c.tiktokAlts : []).map(alt => `<a href="${alt.url}" target="_blank" rel="noopener" class="flex items-center justify-between border-b-2 border-navy/15 pb-3 hover:border-burnt"><span class="font-mono text-[10px] uppercase tracking-[0.18em] text-burnt">tiktok · 2nd</span><span class="font-display font-semibold text-xl lowercase">${alt.handle} ↗</span></a>`).join('')}
